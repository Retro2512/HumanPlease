...
export async function publishRouteStats(env: Env, stats: RouteStats[]): Promise<void> {
  ...
  if (!/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(env.GITHUB_REPOSITORY)) throw new Error('github_repository_invalid');
  if (!/^[A-Za-z0-9._/-]+$/.test(env.GITHUB_BASE_BRANCH) || env.GITHUB_BASE_BRANCH.includes('..')) {
    throw new Error('github_base_branch_invalid');
  }
  if (!/^\d+$/.test(env.GITHUB_APP_ID) || !/^\d+$/.test(env.GITHUB_INSTALLATION_ID)) {
